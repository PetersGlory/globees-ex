...
import { BASE_URL, GENERAL_URL, sendpush } from '../../config/api/Index'
...
const LoginScreen = ({navigation}) => {
    ...
    const handleLogin = () =>{
        ...
        if(logins.email !== "" && logins.password !== ""){
            axios.post(`${BASE_URL}/authenticate`,logins).then((result)=>{
    ...
    <SafeAreaView style={{
      ...
      <ScrollView style={tw`h-full w-full ${Platform.OS == "ios" ? "p-5 pt-15" : "" }`}>
        ...
        <View style={tw`mt-[30px]`}>
            ...
            <Text style={tw`text-gray-400 text-[14px] mt-[8px]`}>Welcome to Globees Ex, Sign in to your account</Text>
        </View>
        ...
        <View style={tw`mt-5 w-full flex flex-col items-center`}>
            ...
            <TouchableOpacity onPress={()=>navigation.push("RegistrationScreen")} style={tw`w-full p-4 mt-[12px] border border-gray-300 items-center rounded-2xl`}>
                <Text style={{
                    ...tw`text-[16px] font-medium`,
                    color:'#133A64'
        ...
        <View style={tw`mt-3 w-full flex flex-col items-center pt-5`}>
            ...
            {isBiometricSupported && fingerprint ? (                
                ...
                <View>
                    <Text style={tw`text-center text-[13px] text-gray-700`}>Fingerprint not supported.</Text>
                </View>
            )}
        </View>
      </ScrollView>
