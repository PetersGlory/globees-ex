...
const LoginScreen = ({navigation}) => {
    ...
    const handleLogin = () =>{
        ...
        setMessage('Loading...')
        setLoading(true);
        if(logins.fullname !== "" && logins.phone !== "" && logins.email !== "" && logins.password !== ""){
            axios.post(`${BASE_URL}/registration`,logins).then((result)=>{
    ...
    <SafeAreaView style={tw`flex-grow w-full h-full bg-white p-5 pt-10`}>
      <ScrollView style={tw`h-full w-full ${Platform.OS === "ios" ? "p-5 pt-10" : ""}`}>
        ...
        <View style={tw`mt-[20px]`}>
            <Text style={tw`text-blue-800 text-[24px] font-medium`}>Create a Globees Ex account</Text>
        </View>

        {/* Forms */}
        <View style={tw`mt-[25px] w-full`}>
            <TextInput placeholder='Full Name' value={logins.fullname} onChangeText={(val)=>{
                setLogins({
                    ...logins,
                    fullname: val
                })
            }} keyboardType="default" style={tw`p-[16px] rounded-2xl border border-gray-200`} />
            <TextInput placeholder='Phone Number' value={logins.phone} onChangeText={(val)=>{
                setLogins({
                    ...logins,
                    phone: val
                })
            }} keyboardType="phone-pad" style={tw`p-[16px] rounded-2xl border border-gray-200 mt-5`} />
            <TextInput placeholder='Email' value={logins.email} onChangeText={(val)=>{
                setLogins({
                    ...logins,
                    email: val
                })
            }} keyboardType="email-address" style={tw`p-[16px] rounded-2xl border border-gray-200 mt-5`} />
        ...
        <View style={tw`mt-8 w-full flex flex-col items-center`}>
            ...
            <TouchableOpacity onPress={()=>navigation.push("LoginScreen")} style={tw`w-full p-4 mt-[14px] border border-gray-300 items-center rounded-2xl`}>
                <Text style={{
