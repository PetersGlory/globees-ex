    ...
    <SafeAreaView style={tw`flex-grow w-full h-full bg-gray-100 p-5 pt-10`}>
        ...
        <View style={tw` w-full h-full ${Platform.OS == "ios" && "pl-5 pr-5"}`}>
            ...
            {transactions.length > 0 ? (
                <ScrollView style={tw`mt-2 w-full h-[90%]`}
                    refreshControl={<RefreshControl refreshing={refresh} onRefresh={handleRefresh} />}
                    >
                
                    {transactions && transactions.map((transact, i)=>(
                        <View style={tw`rounded-lg p-3 mt-3 w-full flex flex-row bg-white justify-between items-center`} key={i}>
                            <View style={tw`flex-1 flex flex-row items-center`}>
                                <Icon name='md-notifications-outline' style={tw`rounded-full w- p-3 text-blue-800 bg-gray-100 items-center text-center`} />
                                <View style={tw`pl-1 w-full`}>
                                    <Text style={tw`text-blue-900 text-[12px] font-medium`}>{transact?.title}</Text>
                                <Text style={tw`text-gray-500 text-[11px]`}>{transact?.contents}</Text>
                                </View>
                            </View>
                ...
                >
                    ...
                    <View style={tw`items-center justify-center mt-5`}>
                        ...
                        <Text style={tw`text-center mt-2 text-gray-600`}>Notification is empty</Text>
                    </View>
                </ScrollView>
            )}
        </View>
