...
import { View, Text, SafeAreaView, TextInput, ScrollView, TouchableOpacity, Image, Linking } from "react-native";
  ...
  if (Platform.OS === "ios") {
    ...
    return (
      <SafeAreaView style={tw`flex-grow bg-white w-full h-full`}>
        ...
        <View style={tw`w-full px-4 flex-1 items-center justify-center gap-2`}>
          <TouchableOpacity style={tw`w-auto p-3 rounded-lg border border-[#25d366] bg-[#25d366]`} onPress={() => {
            ...
            <Ionicons name="logo-whatsapp" size={50} color={"#FFFFFF"} />
          </TouchableOpacity>
          <Text style={tw`text-center`}>
            🚨 This service is coming soon on iOS!
          </Text>
          <Text style={tw`text-center`}>
            If you have any queries or need assistance converting your crypto to cash, kindly reach out to our support team on WhatsApp.
          </Text>
          <Text style={tw`text-center`}>
            Click here to get in touch with support.
          </Text>
        </View>
    ...
    return (
      <SafeAreaView style={tw`flex-grow bg-white w-full h-full`}>
        <StatusBar style="dark" />
        <ScrollView
          style={tw`h-full w-full ${"p-5 h-full w-full"
            }`}
        >
          <CustomHeader title={"Crypto Exchange"} />
